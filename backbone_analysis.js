...
(function() {
    ...
    var Events = Backbone.Events = {
        ...
        on: function(events, callback, context) {
            ...
            // 注意 calls和 this._callbacks指向了同一个对象~后续往calls中添加各个event属性~也就等价于向 this._callbacks添加属性
            // 所以这里 this._callbacks还有另一个作用比如我们 new了一个扩展的 model对象~第一次调用on方法给这个对象绑定了事件“first”
            // 之后再次调用on方法给此model对象绑定事件“second”~此时this._callbacks就不再为空~而是保存了之前绑定的事件的信息了
            // 就可以通过this._callbacks得到上一次绑定 “first”事件的相关信息并在此基础上继续追加事件
            // 而如果仅仅通过calls这个局部变量~是无法保存之前的绑定信息的
            ...
            while (event = events.shift()) {
                ...
                node.next = tail = {};
                // 注意这里的逻辑~我们从第一次给mode实例绑定事件开始分析：
                // 第一次绑定的时候 calls[event]为空，所以这里相当于全部初始化，局部变量 node和 tail也为空~起初看这两个局部变量是之间是没有任何联系的~仅仅是两个空对象
                // 但是因为上面这一句 node.next = tail = {}，使得 node.next和tail指向了同一个对象
                // 先看一下在最后组装 calls[event]的时候~我们绑定第一个事件处理函数得到的第一个结构如下：
                /**
                 * {
                 *       tail: {},
                 *       next: {
                 ...
                 *   }
                 */
                // 单纯从这个结构来看~只看到 tail和 next都是空对象~容易忽略他们其实指向了同一个对象
                // 这点很关键~因为在下次再次绑定事件的时候~上面的 list = calls[event];已经有值了~假设此时是第二次绑定同个事件的第二个事件处理函数
                // 那么所以局部变量 node = list.tail，此时也是一个空对象，但是和第一次不同~此时对 node进行操作，向其中添加属性~不仅会影响到 list.tail，
                // 还会影响到 list.next中的 next~原因就是以为第一次的时候我们写了 node.next = tail = {};
                // 这就使得第一次得到的结构中 tail和 next中的 next(注意这里容易搞错~是 tail和 next中的 next而不是 tail和与他并列的那个 next)
                // 相互关联 ~而新循环中声明的局部变量 node又与 tail指向同一个变量~
                // 所以局部变量 node~第一次结构中的 tail和第一次结构中的 next中的 next都指向了同一个对象 ~
                // 下面再对局部变量 node进行操作~会同时影响这三个变量
                // 所以在第二次重组 calls[event]之前~我们得到的 list的结构如下：
                /**
                 * {
                 *      tail: {
                 *          next： {},
                 ...
                 *  }
                 */
                // 然后局部变量 tail此时指向最里面的那个 next对象~然后重组后得到
                /**
                 * calls[event] = {
                 *      tail: {}, // 此处 tail字段的值与最里面的 next字段的值 指向同一个对象
                 *      next: {
                 *          callback: { Function },
                 *          context: { Object },
                 *          next: {
                ...
                // 最后~因为 calls和 this._callbacks指向同一个对象~所以重组完~this._callbacks也就保存了20:41:3事件列表了
            }
            // 返回当前对象, 方便进行方法链调用
            return this;
        },
        ...
        off: function(events, callback, context) {
            ...
            // 解析需要移除的事件列表
            // - 如果指定了events, 则按照 eventSplitter 对事件名进行解析
            // - 如果没有指定events, 则解析已绑定所有事件的名称列表
            events = events ? events.split(eventSplitter) : _.keys(calls);
            // 比如当前model实例对象已经绑定了'asd', 'asd2'两个事件的处理函数~calls结构如下：
            /**
             *  {
             *      'asd': {
             *          tail: {},
             ...
             *   }
             */
            // 然后在调用 off方法的时候假设没有传递 events参数
            // 则会取得 asd和 asd2字段对应的对象进行操作
            ...
            while (event = events.shift()) {
                ...
                tail = node.tail;
                // 遍历事件中的所有回调对象
                // 注意这里使用的是严格比较~因为上面我们绑定事件的时候已经保证了 tail始终和最里面的 next指向同一个变量
        ...
        trigger: function(events) {
            ...
            events = events.split(eventSplitter);
            // 将 trigger从第2个之后的参数, 记录到 rest变量, 将依次传递给回调函数
            // 因为调用场景是 myModel.trigger('event1 event2', 'Xx');这里第二个以及第二个以后的参数都是传递给事件处理函数的参数
            // 这里 slice函数就是 Array.prototype.slice~我们在最开始用局部变量 slice做了缓存以避免重复查找原型链
            ...
            while (event = events.shift()) {
                ...
                if (node = all) { // 注意这里是赋值语句不是判断语句~所以 if 条件块中的语句一定会被执行
                    tail = node.tail;
                    // 与调用普通事件的回调函数不同之处在于, all事件会将当前调用的事件名作为第一个参数传递给回调函数
                    // [event]目的是为了将event转为数组好调用 concat 方法~执行后结果就是在 rest数组的开头追加了当前调用的事件名
                    // 个人感觉其实上面通过 slice.call(arguments, 1);得到的 rest已经是数组了~直接 rest.unshift(event)不就好了嘛 >_<
